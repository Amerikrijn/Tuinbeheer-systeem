-- SECURE SUPABASE SQL SCHEMA - EXACT MATCH MET JOUW DATABASE
-- Versie: FINAL - CORRECTE UUID/TEXT TYPES
-- Datum: 2024
...
✅ Exact match met jouw database data types:
   - gardens.id = UUID
   - plant_beds.id = TEXT, garden_id = UUID  
   - plants.id = UUID, plant_bed_id = TEXT
   - user_garden_access.garden_id = UUID
✅ Alle foreign key constraints correct
✅ Behoudt alle bestaande kolommen en constraints
...
✅ GEEN "USING (true)" policies meer

DEPLOYMENT:
- 100% backward compatible met jouw database
- Bestaande gebruikersbeheer blijft werken
- Admin interface blijft functioneren  
- Alle bestaande queries blijven werken
*/