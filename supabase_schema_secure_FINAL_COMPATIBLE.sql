-- SECURE SUPABASE SQL SCHEMA - FINAL COMPATIBLE VERSION
-- Versie: FINAL COMPATIBLE - Respecteert bestaande audit_log tabel
-- Datum: 2024
...
-- ===========================================
-- CORE TABLES (EXACT TYPES MATCH)
-- ===========================================

-- Gardens table (id = UUID)
...
-- Plant beds table (id = TEXT, garden_id = UUID)
...
-- Plants table (id = UUID, plant_bed_id = TEXT)
...
-- Logbook entries table (plant_bed_id = TEXT, plant_id = UUID)
...
-- ===========================================
-- USER MANAGEMENT TABLES (BESTAANDE - BEHOUDEN!)
-- ===========================================
...
-- User garden access table (BESTAANDE - garden_id = UUID)
...
-- NOTE: audit_log RLS wordt gehandeld in secure_audit_migration.sql
...
-- Plants policies (plant_bed_id = TEXT)
...
-- NOTE: audit_log permissions worden gehandeld in secure_audit_migration.sql
...
CREATE INDEX IF NOT EXISTS idx_logbook_plant_bed_id ON logbook_entries(plant_bed_id);
...
-- Set created_by voor bestaande records
DO $$
DECLARE
    admin_user_id UUID;
BEGIN
    ...
    -- Update bestaande records (ZONDER audit triggers te activeren)
...
/*
SECURE DEPLOYMENT STRATEGIE:
✅ STAP 1: Run dit script (supabase_schema_secure_FINAL_COMPATIBLE.sql)
   - Voegt created_by kolommen toe
   - Implementeert secure RLS policies
   - Database hardening
   - Performance indexes
   - GEEN audit_log wijzigingen

✅ STAP 2: Run secure_audit_migration.sql
   - Maakt backup van bestaande audit_log
   - Update audit triggers compatibel met action_type kolom
   - Implementeert audit RLS policies
   - Performance indexes voor audit_log

DATABASE COMPATIBILITY:
✅ Exact match met jouw database data types
✅ Behoudt bestaande audit_log structuur  
✅ Alle foreign key constraints correct
✅ Compatible met bestaande frontend code
✅ Geen data verlies risico

SECURITY COMPLIANCE:
✅ DNB Good Practice: Least privilege via RLS policies  
✅ NCSC ICT-beveiligingsrichtlijnen: Eigenaarschap-gebaseerd
✅ ASVS Level 2: Proper authorization and audit logging
✅ Database hardening met timeouts
