-- SECURE SUPABASE SQL SCHEMA - GITHUB COMPATIBLE
-- Versie: 3.0 - EXACT MATCH MET GITHUB VERSIE
-- Datum: 2024
...
-- ===========================================
-- CORE TABLES (EXACT GITHUB STRUCTURE)
-- ===========================================

-- Gardens table (exact GitHub match)
...
-- Plant beds table (exact GitHub match)
...
-- Plants table (exact GitHub match)
...
-- Logbook entries table (exact GitHub match)
...
-- ===========================================
-- USER MANAGEMENT TABLES (GITHUB COMPATIBLE)
-- ===========================================
...
CREATE TABLE IF NOT EXISTS public.user_garden_access (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  user_id UUID REFERENCES public.users(id) ON DELETE CASCADE NOT NULL,
  garden_id TEXT REFERENCES public.gardens(id) ON DELETE CASCADE NOT NULL,
  granted_by UUID REFERENCES public.users(id),
  granted_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
-- Plants policies (GITHUB COMPATIBLE: plant_bed_id is UUID, plant_beds.id is TEXT)
...
✅ Exact match met GitHub schema structuur
✅ Correcte data types (TEXT vs UUID)
...
DEPLOYMENT:
- 100% backward compatible met GitHub versie
