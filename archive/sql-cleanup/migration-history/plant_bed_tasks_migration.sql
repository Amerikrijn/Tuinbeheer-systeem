-- Migration to add plant bed level tasks support
-- This extends the existing tasks table to allow tasks at plant bed level

-- Add plant_bed_id column to tasks table (nullable, since tasks can be either plant-specific or plant bed-specific)
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS plant_bed_id uuid REFERENCES plant_beds(id) ON DELETE CASCADE;
