...
export async function middleware(request: NextRequest) {
  ...
  if (isProtectedRoute) {
    try {
      // BACKWARD COMPATIBLE: Use existing auth method
      ...
      // Get session from cookie (existing method)
      ...
      // Verify session server-side (existing method)
      ...
      // Session timeout check (existing method)
      ...
      // Admin route protection (existing method - uses user metadata)
      if (pathname.startsWith('/admin')) {
        const isAdmin = user.app_metadata?.role === 'admin' || 
                       user.user_metadata?.role === 'admin' ||
                       user.email?.includes('admin') // Fallback for existing setup
        if (!isAdmin) {
