...
/*
TESTOMGEVING SETUP VOLTOOID!

Deze SQL script heeft het volgende opgezet:

📊 DATABASE STRUCTUUR:
- 3 hoofdtabellen: gardens, plant_beds, plants
- Alle benodigde indexen voor performance
- Triggers voor automatische timestamp updates
- RLS policies (open voor testomgeving)
- Nuttige views voor data analysis

🌱 TEST DATA:
- 4 test tuinen (mixed, vegetable, flower, herb)
- 9 plantvakken met verschillende eigenschappen
- 20+ test planten met verschillende statussen
- Realistische test scenario's

🔧 FUNCTIONALITEIT:
- Visual garden designer ondersteuning
- Soft delete functionaliteit
- Bulk position updates
- Status tracking (gezond, aandacht_nodig, ziek, dood, geoogst)
- Harvest date tracking

🚀 GEBRUIK:
1. Run deze SQL in je Supabase test database
2. Gebruik de test data voor ontwikkeling
3. Test alle functionaliteiten met de verschillende statussen
4. Pas RLS policies aan naar jouw authenticatie behoeften

📋 VERIFICATIE:
- Run: SELECT * FROM verify_test_environment();
- Check de summary statistics hierboven
- Test CRUD operaties op alle tabellen

Voor productie gebruik:
- Pas RLS policies aan voor echte authenticatie
- Verwijder test data
- Voeg eigen productie data toe
*/