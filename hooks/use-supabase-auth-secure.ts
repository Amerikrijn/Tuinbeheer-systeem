...
export function useSupabaseAuth(): AuthContextType {
  ...
  // SECURE: Load user profile WITHOUT emergency backdoors
  const loadUserProfile = async (supabaseUser: SupabaseUser, useCache = true): Promise<User> => {
    ...
    try {
      ...
      const databasePromise = supabase
        ...
        .eq('id', supabaseUser.id) // SECURE: Use user ID instead of email
        .eq('status', 'active') // SECURE: Only allow active users
        .single()
      ...
      if (userError || !userProfile) {
        // SECURE: No emergency backdoors - proper error handling
        console.error('User profile not found or inactive:', userError)
        throw new Error('Access denied: User not found in system or account inactive. Contact admin.')
      }

      // SECURE: Validate role from database
      ...
      const user: User = {
        ...
        full_name: userProfile.full_name || supabaseUser.email?.split('@')[0] || 'User',
      ...
      try {
        ...
        if (session?.user) {
          ...
          if (cachedUser && cachedUser.id === session.user.id) {
            ...
            const userProfile = await loadUserProfile(session.user, false)
            setState({
              user: userProfile,
              session,
              loading: false,
  ...
  const refreshUser = async (): Promise<void> => {
    if (state.session?.user) {
      const userProfile = await loadUserProfile(state.session.user, false)
