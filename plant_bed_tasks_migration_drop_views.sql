-- Migration to add plant bed level tasks support (DROP AND RECREATE VIEWS)
-- This extends the existing tasks table to allow tasks at plant bed level
-- Drops existing views first to avoid conflicts

-- Drop existing views first to avoid column conflicts
DROP VIEW IF EXISTS weekly_tasks;
DROP VIEW IF EXISTS tasks_with_plant_info;

-- Add plant_bed_id column to tasks table with TEXT type (matching plant_beds.id)
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS plant_bed_id TEXT;

-- Add the foreign key constraint with correct TEXT type
...
-- Recreate the tasks_with_plant_info view to include plant bed tasks
CREATE VIEW tasks_with_plant_info AS
SELECT 
    t.*,
    CASE 
...
-- Recreate the weekly_tasks view to include plant bed tasks
CREATE VIEW weekly_tasks AS
SELECT 
    t.*,
    CASE 
        WHEN t.plant_id IS NOT NULL THEN p.name
