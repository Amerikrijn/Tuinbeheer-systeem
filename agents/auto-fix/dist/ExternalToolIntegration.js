...
exports.ExternalToolIntegration = void 0;
...
class ExternalToolIntegration {
     ...
     */
    async analyzeWithExternalTools(filePath) {
        const results = {};
        try {
     ...
     */
    async analyzeWithSonarQube(filePath) {
        try {
     ...
     */
    async analyzeWithCodeClimate(filePath) {
        try {
     ...
     */
    async analyzeWithGitHub(filePath) {
        try {
     ...
     */
    async analyzeWithGitLab(filePath) {
        try {
     ...
     */
    async saveSonarQubeResults(filePath, results) {
        try {
     ...
     */
    async saveCodeClimateResults(filePath, results) {
        try {
     ...
     */
    async saveGitHubResults(filePath, results) {
        try {
     ...
     */
    async saveGitLabResults(filePath, results) {
        try {
     ...
     */
    async getOverallQualityScore(results) {
        let totalScore = 0;
        let toolCount = 0;
        // SonarQube score
     ...
     */
    calculateSonarQubeScore(results) {
        let score = 100;
     ...
     */
    calculateCodeClimateScore(results) {
        let score = 100;
    ...
    async generateQualityReport(results) {
        ...
        // SonarQube results
        if (results.sonarQube) {
            report.push('', '### 🟢 SonarQube', `- **Quality Gate:** ${results.sonarQube.qualityGate}`, `- **Bugs:** ${results.sonarQube.bugs}`, `- **Vulnerabilities:** ${results.sonarQube.vulnerabilities}`, `- **Code Smells:** ${results.sonarQube.codeSmells}`, `- **Coverage:** ${results.sonarQube.coverage.toFixed(1)}%`, `- **Duplications:** ${results.sonarQube.duplications.toFixed(1)}%`, `- **Maintainability:** ${results.sonarQube.maintainability}`, `- **Reliability:** ${results.sonarQube.reliability}`, `- **Security:** ${results.sonarQube.security}`);
        }
        // CodeClimate results
        if (results.codeClimate) {
            report.push('', '### 🔵 CodeClimate', `- **GPA:** ${results.codeClimate.gpa.toFixed(1)}/4.0`, `- **Issues:** ${results.codeClimate.issuesCount}`, `- **Test Coverage:** ${results.codeClimate.testCoverage.toFixed(1)}%`, `- **Maintainability:** ${results.codeClimate.maintainability}`, `- **Technical Debt:** ${results.codeClimate.technicalDebt.toFixed(0)} minutes`, `- **Complexity:** ${results.codeClimate.complexity}`);
        }
        // GitHub results
        if (results.github) {
            report.push('', '### 🐙 GitHub', `- **Repository:** ${results.github.repository}`, `- **Branch:** ${results.github.branch}`, `- **Open Issues:** ${results.github.issues.length}`, `- **Open PRs:** ${results.github.pullRequests.length}`, `- **Actions:** ${results.github.actions.length}`);
        }
        // GitLab results
        if (results.gitlab) {
            report.push('', '### 🦊 GitLab', `- **Project:** ${results.gitlab.project}`, `- **Branch:** ${results.gitlab.branch}`, `- **Open Issues:** ${results.gitlab.issues.length}`, `- **Open MRs:** ${results.gitlab.mergeRequests.length}`, `- **Pipelines:** ${results.gitlab.pipelines.length}`);
        }
        report.push('', '## 📈 Recommendations', '', '### 🚨 Critical Issues', this.generateCriticalRecommendations(results), '', '### 🔧 Improvement Suggestions', this.generateImprovementSuggestions(results), '', '---', '*Generated by Auto-Fix Agent v2.0*');
        return report.join('\n');
    }
    /**
     * Generate critical recommendations
     */
    generateCriticalRecommendations(results) {
...
exports.ExternalToolIntegration = ExternalToolIntegration;
//# sourceMappingURL=ExternalToolIntegration.js.map