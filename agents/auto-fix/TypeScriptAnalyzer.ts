...
export class TypeScriptAnalyzer {
   ...
   */
  async analyzeFile(filePath: string): Promise<TypeScriptAnalysis> {
   ...
   */
  async generateFixes(analysis: TypeScriptAnalysis): Promise<CodeFix[]> {
    const fixes: CodeFix[] = []
  ...
  private async generateFixForDiagnostic(
    ...
    const sourceFile = analysis.ast as ts.SourceFile
   ...
   */
  private async generateFixByCode(
    code: number, 
    node: ts.Node, 
   ...
   */
  private fixPropertyAccess(node: ts.Node, sourceFile: ts.SourceFile): string | null {
   ...
   */
  private fixTypeMismatch(node: ts.Node, sourceFile: ts.SourceFile): string | null {
          ...
          if (index < parameters.length) {
            ...
            if (paramType && ts.isParameter(paramType) && paramType.type) {
              ...
              fixedCode = fixedCode.replace(
                ...
                `(${arg.getText(sourceFile)} as ${typeText})`
              )
            }
          }
        })
   ...
   */
  private fixUndefinedVariable(node: ts.Node, sourceFile: ts.SourceFile): string | null {
   ...
   */
  private fixTypeAssignment(node: ts.Node, sourceFile: ts.SourceFile): string | null {
   ...
   */
  private fixImplicitAny(node: ts.Node, sourceFile: ts.SourceFile): string | null {
   ...
   */
  private findSimilarNames(name: string, sourceFile: ts.SourceFile): string[] {
    const names: string[] = []
   ...
   */
  private inferReturnType(node: ts.FunctionDeclaration | ts.MethodDeclaration): string | null {
    if (this.checker) {
   ...
   */
  private inferParameterType(node: ts.ParameterDeclaration): string | null {
    if (this.checker) {
   ...
   */
  private extractSymbols(sourceFile: ts.SourceFile): TypeScriptSymbol[] {
   ...
   */
  private extractImports(sourceFile: ts.SourceFile): TypeScriptImport[] {
   ...
   */
  private extractExports(sourceFile: ts.SourceFile): TypeScriptExport[] {
   ...
   */
  private extractTypeInfo(sourceFile: ts.SourceFile): TypeScriptTypeInfo {
    return {
      type: 'mixed',
      isPrimitive: false,
      isUnion: false,
  ...
  private extractDocumentation(node: ts.Node): string[] {
    ...
    if (node.parent && ts.isVariableStatement(node.parent)) {
      ...
      if (trivia > 0) {
        const triviaText = node.parent.getSourceFile().text.substring(
          node.parent.getStart() - trivia,
   ...
   */
  private convertDiagnostics(diagnostics: ts.Diagnostic[]): TypeScriptDiagnostic[] {
   ...
   */
  private mapSeverity(category: ts.DiagnosticCategory): 'low' | 'medium' | 'high' | 'critical' {
    switch (category) {
   ...
   */
  private createCompilerHost(config: ts.ParsedCommandLine): ts.CompilerHost {
    return {
