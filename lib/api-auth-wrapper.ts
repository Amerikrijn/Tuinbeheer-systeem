...
export async function withBankingAuth<T>(
  handler: (request: NextRequest, params: unknown, user: AuthenticatedUser | null) => Promise<NextResponse<T>>,
  options: AuthOptions = {}
) {
  return async (request: NextRequest, params?: unknown): Promise<NextResponse<T>> => {
